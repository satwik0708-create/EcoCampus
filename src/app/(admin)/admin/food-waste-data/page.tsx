...
import { listFoodWasteRecords } from "@/lib/services/records";
...
export const metadata = { title: "Food Waste Data" };

const CATEGORY_OPTIONS = enumOptions(FOOD_CATEGORY_META).map((option) => ({
  value: option.value,
  label: option.label,
}));

export default async function FoodWasteDataPage({
  searchParams,
}: {
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        description="Every food waste record on campus, with the meal and avoidability that explain it."
      />
...
async function FoodTable({
  ...
  const filters = parsed.success ? parsed.data : { page: 1, pageSize: 20 as const };

  const data = await listFoodWasteRecords({
    page: filters.page,
  ...
  if (data.rows.length === 0) {
    return (
      <EmptyState
        ...
        description="No food waste records match these filters. Widen the date range or clear the filters to see everything."
      />
    );
  }
            ...
            <TableRow key={row.id}>
              ...
              <TableCell>
                <Badge variant="outline">
                  {FOOD_CATEGORY_META[row.foodCategory].label}
                </Badge>
              </TableCell>
              <TableCell className="text-muted-foreground hidden sm:table-cell">
                {MEAL_TYPE_META[row.mealType as keyof typeof MEAL_TYPE_META].label}
              </TableCell>
              <TableCell>
